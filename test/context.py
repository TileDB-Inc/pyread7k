...
def check_for_s7kfile(max_recursions=3):
    if max_recursions==0:
        raise RecursionError("You failed to specify correct paths in a reasonable time. Try again.")
    ...
    elif ((bf_filepath.endswith(".s7k") and iq_filepath.endswith(".s7k")) 
        and all(map(os.path.exists, [bf_filepath, iq_filepath]))):
        return True
...
iq_filepath = os.environ.get("iq_filepath", None)
if any([bf_filepath is None, iq_filepath is None]):
    raise ValueError("Unexpected ValueError! s7k_filepath is for some reason not defined!")