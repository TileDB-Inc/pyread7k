...
gain_transform = np.arange(ping.tvg.data["gain"].shape[0])**3/ 10**(ping.tvg.data["gain"] / 20)
compensated_amp = ping.beamformed.data["amp"] * gain_transform[:, None]

# Find max intensity
point_range, point_beam = np.unravel_index(np.argmax(compensated_amp, axis=None), compensated_amp.shape)

# Translate indices to real-world metrics
point_direction = ping.beam_geometry.data["horz_angle"][point_beam]
point_distance = point_range * ping.sonar_settings.header["sound_velocity"] \
                 / ping.sonar_settings.header["freq"]

print("Detection at %.2f meters in direction %.2f rad" % (point_distance, point_direction))
