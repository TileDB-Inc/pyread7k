...
_ElementTypes = namedtuple('_ElementTypes', [
    ...
    'c8', 'i8', 'u8',  # 3 int types of 1 byte
    'i16', 'u16',  # 2 int types of 2 bytes
    'i32', 'u32',  # 2 int types of 4 bytes
    'i64', 'u64',  # 2 int types of 8 bytes
    'f32',  # 1 float type of 4 bytes
    'f64'  # 1 float type of 4 bytes
])
elemT = _ElementTypes(
    **dict(zip(_ElementTypes._fields, _ElementTypes._fields)))
...
class DataBlock(metaclass=abc.ABCMeta):
    ...
    def read(self, source: io.RawIOBase, count=1):
        ...
        for _ in range(count):
            ...
            offset = 0
            for name, (_, count) in elements_zip:
                if isinstance(name, str):
                    dict_read[name] += unpacked[
                        offset:(offset + count)]
                offset += count
        return {k: (v[0] if len(v) == 1 else tuple(v))
