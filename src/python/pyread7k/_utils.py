...
def read_file_header(
        source: io.RawIOBase) -> _DataParts:
    return _record(7200).read(source)


def read_file_catalog(
        source: io.RawIOBase,
        file_header: _DataParts) -> _DataParts:
...
def get_record_offsets(
        type_id: int,
        file_catalog: _DataParts) -> tuple:

    cat_zip = zip(
        file_catalog.rd['offset'],
        file_catalog.rd['record_type_id'])

    return tuple(offset for offset, _type_id in cat_zip
                 if _type_id == type_id)


def get_record_count(
        type_id: int,
        file_catalog: _DataParts) -> int:
...
def read_records(
        ...
        source: io.RawIOBase,
        file_catalog: _DataParts,
        *, first_idx=0, count=None) -> _DataParts:

    return tuple(records(
        type_id, source, file_catalog,
        first_idx=first_idx, count=count))
...
def export_catalog(filename: str, file_catalog: _DataParts):
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        for idx, (type_id, offset, size) in enumerate(zip(
                file_catalog.rd['record_type_id'],
                file_catalog.rd['offset'],
